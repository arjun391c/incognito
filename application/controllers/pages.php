...
class Pages extends CI_Controller {
    ...
    public function login()
    {
        ...
        $data['user'] = array();
	$demo = $this->facebook->is_authenticated();

	$REDIR_URL =base_url("pages/login");
	$code = $_GET["code"];
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, "https://graph.facebook.com/v3.2/oauth/access_token?client_id=541738999785266&redirect_uri=".$REDIR_URL."&client_secret=68121c8479763b19af2ea5f3bd509c80&code=".$code);
	...
	$this->session->set_userdata('access_token', $access_token);
        ...
        if ($tok) 
        {
            try 
            {
                $data['user_profile'] = $this->facebook->request('get','/me?fields=id,name', $access_token);  //??
            } 
    ...
    public function create()
    {
        ...
        try
        {
		$userdata=$this->facebook->request('get','/me', $this->session->userdata('access_token'));
		$userdata['link'] = 'https://fb.com/'.$userdata['id'];
        }
        catch (FacebookApiException $e) 
        {
            $userdata=FALSE;
       ...
       $this->load->library('facebook');
        ...
        if($type!='pass'){
            $msg = array(
                ...
                'link' => 'https://incognito.hestia.live'
                );
        }else{
            $msg = array(
                'access_token'  => $token,
    ...
    {
        redirect('https://www.facebook.com/incognito.hestia','location');
        /*$this->load->library('session');
        $this->load->model('users_model');
